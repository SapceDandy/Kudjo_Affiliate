...
export default function BusinessHome() {
  const user = { uid: 'demo_business_user' }; // Mock user for now
  ...
  // Simple filter state instead of complex useSearchFilter
  ...
  const filteredRequests = realRequests.filter((req: any) => 
    req.influencer?.toLowerCase().includes(requestsSearch.toLowerCase())
  );
  
  const filteredPrograms = realPrograms.filter((prog: any) => 
    prog.influencer?.toLowerCase().includes(programsSearch.toLowerCase())
  );
  
  const refetchRequests = async () => {};
  const refetchPrograms = async () => {};
  const processPayout = async (ids: string[]) => {};
  ...
  useEffect(() => {
    ...
    const run = async () => {
      if (!user?.uid) return;
      try {
        const snap = await fetch(`/api/business/tier-settings?businessId=${user.uid}`);
  ...
  return (
    <>
                  ...
                  {r.status === 'pending' ? (
                    <>
                      ...
                      <Button 
                        ...
                        onClick={async () => {
                          try {
                            await fetch(`/api/business/requests/${r.id}`, {
                              method: 'PATCH',
                              headers: {
                                'Content-Type': 'application/json',
                              },
                    ...
                    <div className="text-sm text-muted-foreground">
                      {r.status === 'approved' ? 'Approved' : 
                       ...
                       r.status === 'closed' ? 'Closed' : 'Awaiting Response'}
                    </div>
                  )}
                </div>
              </CardContent>
      ...
      <div className="space-y-3">
            ...
            <Button 
              onClick={async () => {
                ...
                await Promise.all(Object.keys(map).map(async inf => {
                  ...
                  try {
                    await fetch(`/api/business/payouts`, {
                      method: 'POST',
                      headers: {
                        'Content-Type': 'application/json',
                      },
                      body: JSON.stringify({ id: recId, businessId: user?.uid || 'unknown', influencer: inf, amountCents: map[inf].amount, programIds: map[inf].programIds, createdAt: ts, status: 'paid' }),
                    });
                  } catch {}
                }));
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {Array.from(new Set(realPrograms.map((p: any) => p.influencer))).map((inf) => {
      ...
      {counterReqId && (
        <CounterOfferDialog 
          ...
          onCounter={async (payload) => {
            await fetch(`/api/business/requests/${counterReqId}`, {
              method: 'PATCH',
              headers: {
                'Content-Type': 'application/json',
              },
              body: JSON.stringify({ status: 'countered', ...payload }),
            });
      ...
      {messageProgramId && (
        <MessageDialog programId={messageProgramId} open={true} onClose={() => setMessageProgramId(null)} onSend={async (msg) => {
          try { await fetch(`/api/business/messages`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
            body: JSON.stringify({ programId: messageProgramId, bizId: user?.uid || 'unknown', message: msg }),
          }); } catch {}
        }} />
      )}
    </div>
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Make a counter offer to this influencer request with custom terms.
          </DialogDescription>
        </DialogHeader>
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            View detailed performance metrics and analytics for this offer.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4">
          {loading ? (
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-xl">
        <DialogHeader>
          ...
          <DialogDescription>
            View detailed information about this active program and its participants.
          </DialogDescription>
        </DialogHeader>
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Send a direct message to this influencer about collaboration opportunities.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-3">
...
function TierDefaultsDialog({ open, onClose, onSave, bizId }: { open: boolean; onClose: () => void; onSave: (settings: TierSettings) => void; bizId: string }) {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      setLoading(true);
      try {
        const snap = await fetch(`/api/business/tier-settings?businessId=${bizId}`);
        const data = await snap.json();
        if (!cancelled && data) {
          setSettings(data);
        }
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          <DialogTitle>Tier Defaults</DialogTitle>
          <DialogDescription>
            Configure default commission rates and discount settings for each influencer tier.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4">
...
// Add the missing closing component at the end
