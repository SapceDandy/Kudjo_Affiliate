...
async function seedCollection(collectionName: string, data: any) {
  ...
  for (const [docId, docData] of Object.entries(data)) {
    ...
    const processedData = convertDatesToTimestamps(docData as any);
...
async function seedNestedCollection(collectionName: string, data: any) {
  if (!data) {
    console.log(`  Skipping ${collectionName} - no data provided`);
    return;
  }
  ...
  for (const [parentId, nestedData] of Object.entries(data)) {
    for (const [docId, docData] of Object.entries(nestedData as any)) {
